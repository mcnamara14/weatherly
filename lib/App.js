  ...
  constructor() {
	...
	getLocalStorage() {
		if (window.localStorage.location) {
			...
			this.updateWeather(retrievedLocation.city, 
												retrievedLocation.stateUSA);
		}
	}
	...
	updateLocation(location) {
		...
		this.setState({city, stateUSA: state});
	...
	updateWeather(city, state) {
		fetch(`http://api.wunderground.com/api/${key}/forecast/
					hourly/forecast10day/conditions/q/${state}/${city}.json`)
		.then(data => data.json())
		.then(data => {
	...
	render() {
		if (this.state.welcome === true) {
			return (
				<WelcomePage updateLocation={this.updateLocation} 
			...
			return (
				...
				<section className="container">
					...
					<SevenHour 
						images={this.images}
						currentCondition=
							{this.state.data.current.currentCondition}
