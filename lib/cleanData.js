export default function cleanData({ current_observation, 
  ...
  return {
    current: {
      ...
      [
        ...
        {prop: parseInt(current_observation.precip_today_in)
                        .toFixed(0),  name: 'Precipitation'},
        {prop: parseInt(current_observation.visibility_mi)
                        .toFixed(0),    name: 'Visibility'},
    ...
    hourly: hourly_forecast.slice(0, 7).map(hourObj => {
              ...
              if (hourObj.FCTTIME.civil.length < 8) {
                hourInteger = hourObj.FCTTIME.civil.slice(0, 1);
              } else {
                hourInteger = hourObj.FCTTIME.civil.slice(0, 2);
              }

              return Object.assign({}, {hour: hourInteger}, 
                  {ampm: hourObj.FCTTIME.ampm}, 
